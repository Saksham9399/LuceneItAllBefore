...
public class Utils {
		...
		String consoleMessage = "Query parsing complete. Total of " + String.valueOf(texts.size()) + " queries parsed. \n \n"; 
        ...
        public ArrayList<DocumentScore> rankDocuments(float[] documentScores) {
                ArrayList<DocumentScore> docScores = new ArrayList<DocumentScore>();

		//System.out.println("Sorting document scores from query result. \n");
	...
	public ParsedDocResults parseFinancialTimesDocuments(File file) throws Exception {
	...
	int docCount = 0;
	while(all != null && index < all.length()){
		index = all.indexOf("<DOCNO>");
		if (index + 7 < all.indexOf("</DOCNO>")){
			docCount = docCount + 1;
			documentNumber = all.substring(index+7, all.indexOf("</DOCNO>"));
			System.out.println("Docnum: " + String.valueOf(documentNumber) + "\n");
			documentNumbers.add(documentNumber);
		}
		else{
			documentNumbers.add("");
		...
		index = all.indexOf("<HEADLINE>");
		System.out.println("Index 1st place: " + String.valueOf(index) + "\n");
		if(index + 10 < all.indexOf("</HEADLINE>")){
			headline = all.substring(index+10, all.indexOf("</HEADLINE>") - 1);
		...
		System.out.println("Second index place: " + String.valueOf(index)+ "\n");

		if(index + 6 < all.indexOf("</TEXT>")){
			text = all.substring(index+6, all.indexOf("</TEXT") - 1);
		...
		if(all.indexOf("</DOC>") > all.length() - 10 ){
			break;
		}
		...
		ArrayList<ArrayList<String>> results = new ArrayList<ArrayList<String>>();
                results.add(documentNumbers); results.add(headlines); results.add(texts);
		
                return new ParsedDocResults(results, docCount);
